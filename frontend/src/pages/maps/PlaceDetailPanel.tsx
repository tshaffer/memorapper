...
import { PlaceType, MrPlaceWithGooglePlace, MrRestaurantReview } from '../../types';
...
const PlaceDetailPanel: React.FC<PlaceDetailPanelProps> = ({
  ...
  const [editPlace, setEditPlace] = useState<MrPlaceWithGooglePlace>({ ...place });
  ...
  const handleSavePlace = async (place: MrPlaceWithGooglePlace) => {
    // onUpdatePlace(place);
    setIsEditing(false);
  };
        ...
        {isEditing ? (
          <Box>
            <PlaceEditor
              ...
              onSubmit={(place: MrPlaceWithGooglePlace) => handleSavePlace(place)}
